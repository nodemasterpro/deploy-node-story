...
update_peers() {
  ...
  PEERS="01f8a2148a94f0267af919d2eab78452c90d9864@story-testnet-peer.itrocket.net:52656,381b10bd04853b375f9a1d42983a0fbfa753e4aa@37.59.22.162:26656,b66b0df0720b38f9405f8a5c50511365ec621b2a@135.181.181.59:62656,7160dec63da82b56e1ce59a93c057c05e361cf85@135.181.117.37:64656,db6791a8e35dee076de75aebae3c89df8bba3374@65.109.50.22:56656,a8d01e154197d799637eca4f0f369dc215db6b70@144.76.111.9:26656,9d34ab3819aa8baa75589f99138318acfa0045f5@95.217.119.251:30900,34c910cde040e983f076195175ed2fe7d447b486@152.53.102.226:26656,dfb96be7e47cd76762c1dd45a5f76e536be47faa@65.108.45.34:32655,ae49103a54f77effa438978ad8a7ba09b6f20da0@144.76.202.120:35656,8b8f8d6fb17b86499c491751975fe27234e60809@65.108.75.52:62656,311cd3903e25ab85e5a26c44510fbc747ab61760@152.53.87.97:36656"
...
# Function to install Geth binary during initial installation
install_geth_binary() {
  echo -e "${BLUE}Installing Geth binary $STORY_GETH_VERSION...${NC}"
  ...
  case $ARCH in
    ...
    aarch64|arm64) BINARY_ARCH="linux_arm64" ;;
    *) 
      echo -e "${YELLOW}Architecture $ARCH not supported for pre-built binaries${NC}"
      return 1
      ;;
  esac
  ...
  if wget -q "$BINARY_URL" -O "geth_${STORY_GETH_VERSION}_${BINARY_ARCH}.tar.gz" 2>/dev/null; then
    if tar -xzf "geth_${STORY_GETH_VERSION}_${BINARY_ARCH}.tar.gz" 2>/dev/null; then
      if [ -f "geth" ]; then
        ...
        echo -e "${GREEN}Geth installation completed! (using pre-built binary)${NC}"
        return 0
      fi
    fi
  fi
  
  # If pre-built binary fails
  echo -e "${YELLOW}Pre-built Geth binary installation failed${NC}"
  cd $HOME
  rm -rf $TEMP_DIR
  return 1
}

# Function to install Story binary during initial installation
install_story_binary() {
  echo -e "${BLUE}Installing Story binary $STORY_VERSION...${NC}"
  ...
  if wget -q "$BINARY_URL" -O "story_${STORY_VERSION}_${BINARY_ARCH}.tar.gz" 2>/dev/null; then
    if tar -xzf "story_${STORY_VERSION}_${BINARY_ARCH}.tar.gz" 2>/dev/null; then
      if [ -f "story" ]; then
        echo -e "${GREEN}Successfully downloaded pre-built Story binary!${NC}"
        ...
        echo -e "${GREEN}Story installation completed! (using pre-built binary)${NC}"
        return 0
      fi
    fi
  fi
  ...
  # If pre-built binary fails
  echo -e "${YELLOW}Pre-built Story binary installation failed${NC}"
  cd $HOME
  rm -rf $TEMP_DIR
  return 1
}
...
install_node() {
  ...
  # Use binary download function instead of compilation
  mkdir -p $HOME/go/bin/
  if ! install_geth_binary; then
    echo -e "${RED}Failed to install Geth binary, falling back to compilation...${NC}"
    cd $HOME
    rm -rf story-geth
    git clone https://github.com/piplabs/story-geth.git
    cd story-geth
  ...
  # Use binary download function instead of compilation
  mkdir -p $HOME/go/bin/
  if ! install_story_binary; then
    echo -e "${RED}Failed to install Story binary, falling back to compilation...${NC}"
    cd $HOME
    rm -rf story
    git clone https://github.com/piplabs/story
    cd story
